...
def action_twist_jump(timer) -> MyRepeatThread:
    # 执行扭身跳命令
    ACTION_twist_jump = MyRepeatThread("ACTION_twist_jump", SendToCommand.perform_action, timer, timer, sfd, target_address, 0x2101020D, 0, 0)
...
def action_continuous_exercise_stop(timer) -> MyRepeatThread:
    # 停止持续运动命令
    ACTION_continuous_exercise_stop = MyRepeatThread("ACTION_continuous_exercise_stop", SendToCommand.perform_action, 0.5, 0.5, sfd, target_address, 0x21010C06, 2, 0)
...
def action_wait(timer) -> MyRepeatThread:
    # 等待
    ACTION_wait = MyRepeatThread("ACTION_wait", None, timer, timer)
    return ACTION_wait
