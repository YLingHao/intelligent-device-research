    ...
    def get_stack_info(self) -> list:
        ...
        for i in range(len(stack) - 1, 0, -1):
            ...
            info_dict = {
                ...
                "line number": frame_info.lineno,
                "function": frame_info.function,
                "code statement": ''.join(frame_info.code_context).strip() if frame_info.code_context else 'No code context'
            }
            stack_info.append(info_dict)
        
        return stack_info
    
    def get_formatted_stack_info(self) -> str:
        """
        获取并格式化调用栈信息，返回格式化后的栈信息字符串
        """
        # 获得堆栈信息的字典列表
        stack_info_list = self.get_stack_info()
        formatted_stack_info = ""

        for info in stack_info_list:
            # 格式化信息
            formatted_stack_info += (
                f"层级 {info['layer']}:\n"
                f"文件: {info['file']}\n"
                f"行号: {info['line number']}\n"
                f"函数: {info['function']}\n"
                f"代码语句: {info['code statement']}\n"
                "----\n"
            )

        return formatted_stack_info
