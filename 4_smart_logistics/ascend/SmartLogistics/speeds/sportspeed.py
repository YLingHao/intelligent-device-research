...
def go_straight(long, speedgear=3, times = None):
    ...
    if times != None and long == 9999:
        if long < 0:
            val = -val
            # speed_per_second_meter = (-1.086e-08) * val ** 2 - 0.0003272 * val - 1.698
            speed_per_second_meter = (-1.5059e-08) * val ** 2 - (4.1944e-04) * val - 2.1804
            long = abs(times * speed_per_second_meter)
        else:
...
def revolve_left_and_right(angle):
    ...
    caller_function = StackTraceParser()
    # print(caller_function.get_formatted_stack_info())

    # 根据栈的追踪，如果旋转的调用执行不同的旋转方式
    if caller_function.get_stack_info()[0]['file'].split('/')[-1] == 'hand_distance_main.py':
        if 8 < abs(angle) < 20:
            if angle < 0:
                val = -10000
            else:
